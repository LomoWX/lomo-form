...
export default {
  ...
  computed: {
    ...
    treePopperClass() {
      return `lomo-tree-select-popper ${this.popperClass || ""}`;
    },
    selectClass() {
      return [
        "lomo-tree-select",
        { "lomo-tree-select-hide-tags": !this.closableTags && this.multiple },
      ];
    },
    selectListeners() {
  ...
  methods: {
    ...
    initScroll() {
      let classname =
        ".lomo-tree-select-popper .el-scrollbar .el-select-dropdown__wrap";
      setTimeout(() => {
        this.$nextTick(() => {
          document
...
<style>
.lomo-tree-select .el-loading-mask {
  background-color: rgba(255, 255, 255, 0.5);
}
.lomo-tree-select .el-loading-mask .el-loading-spinner {
  top: 0px;
  margin-top: 0px;
  display: flex;
  align-items: center;
...
.lomo-tree-select .el-loading-spinner .circular {
  width: 20px;
  height: 20px;
}

.lomo-tree-select-hide-tags .el-tag__close {
  display: none;
}

.lomo-tree-select-popper .filter-item {
  padding: 0px 10px;
  margin-bottom: 10px;
}

.lomo-tree-select-popper
  .el-scrollbar
  .el-scrollbar__view
...
.lomo-tree-select-popper .selected {
  font-weight: inherit;
}
</style>
