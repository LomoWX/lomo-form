<template>
  <el-upload ref="ref" :class="['lomo-upload',{'limit-hide-trigger':isLimitHideTrigger}]" :listType="listType" :autoUpload="autoUpload" :file-list="valueFileList" :before-upload="beforeUpload" :on-success="onSuccess" :on-remove="onRemove" :on-exceed="onExceed" :on-preview="onPreview" :on-error="onError" :on-change="onChange" v-bind="$attrs" v-on="$listeners">
    ...
    onPreview(file) {
      ...
      if (file.url && isPreview && this.$msgbox) {
        ...
        if (includesSuffix(image)) {
          ...
          message = h("video", {
            class: "lomo-upload-preview-video",
            attrs: { src: file.url, controls: true },
          });
        } else if (includesSuffix(audio)) {
          message = h("audio", {
            class: "lomo-upload-preview-audio",
            attrs: { src: file.url, controls: true },
          });
        } else {
        ...
        this.$msgbox({
          title: "附件预览",
          customClass: "lomo-upload-preview-msgbox",
          center: true,
          message,
...
<style>
.lomo-upload.limit-hide-trigger .el-upload {
  display: none;
}

.lomo-upload.limit-hide-trigger .el-upload-list__item:first-child {
  margin-top: 0px;
}

.lomo-upload .el-upload-dragger {
  display: flex;
...
.lomo-upload .el-upload-dragger .el-icon-upload {
  font-size: 50px;
  margin: 0px;
}

.lomo-upload .el-upload-dragger .el-upload__text {
  font-size: 12px;
}

.lomo-upload .el-icon-close-tip {
  display: none !important;
}

.lomo-upload .el-upload-list__item {
  transition: inherit;
}

.lomo-upload-preview-msgbox {
  width: auto;
  padding-bottom: 20px;
  min-width: 300px;
}

.lomo-upload-preview-msgbox .el-message-box__header {
  padding-top: 20px;
}

.lomo-upload-preview-img {
  max-width: 1000px;
  max-height: 600px;
}

.lomo-upload-preview-video {
  max-width: 1000px;
  max-height: 600px;
}

.lomo-upload-preview-doc {
  width: 1000px;
  height: 600px;
}
</style>
