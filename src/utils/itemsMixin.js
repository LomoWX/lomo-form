  ...
  computed: {
    lmItems() {
      return getArrItems(this.items)
        .map((item) => Object.assign(getCustomProps(lmItemProps), item))
        .filter((v) => v.display)
    },
    isDynamic() {
    ...
    dynamicData() {
      ...
      return data.map(row => {
        if (this.isDynamic && row._key_ == undefined) row._key_ = toGenerateId()
        return row
      })
    },
  },
  ...
  methods: {
    ...
    getName(item, deep = true) {
      ...
      if (typeof value === 'string') {
        value = getKebabCase(value)
        if (lmComponentNames.includes("lm" + value)) value = "lm" + value
      }
      return value
    },
    getRequiredAsteriskClass(item, rowIndex) {
      ...
      return item.required || rules.some((v) => v.required) ? "lomo-required-label" : ""
    },
    getDefaultValue(item) {
      ...
      if (name == "lomo-checkbox" && item.options) value = ""
      return value
    },
    getComponentConfig(item) {
      const lmConfig = this.$lomoFormConfig || {}
      const componentConfigFun = lmConfig[item.component] || lmConfig[this.getName(item)]
    ...
    getItem(prop, deep) {
      if (deep) return this.lmItems.find((v) => v.prop == prop)
