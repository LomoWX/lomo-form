<template>
  <el-form-item v-bind="$attrs" :prop="prop" label="" class="lomo-form-item" ref="elFormItem">
    <slot slot="label" name="label">
      <render-component v-if="label" :render="label" />
    </slot>
    <slot>
      <render-component v-if="component.name"
                        :render="component.name"
                        v-bind="component.attrs"
                        v-on="component.on"
                        :is-tag="component.isTag"
                        :ref="prop||'component'" :value="field.v" @input="input">
...
.el-form--label-top .lomo-form-item{
  .el-form-item__label {
...
/*  对某些组件排版进行优化 */
.lomo-form-item.el-form-item--mini {
  .el-slider__runway {
    margin: 12px 0px;
  }
}
