 ...
 * @description：lomo-form
 * @update: 2022/12/1 14:31
 */
...
export default {
  ...
  methods: {
    ...
    getItem(prop, deep) {
      return this.$refs.lmLayout.getItem(prop, deep)
    },
  ...
  render(h) {
    ...
    const config = (this.$lomoFormConfig || {})['form'] || {}
    const FormWithLayoutOption = Object.assign({}, config, option)
    const layoutName = "lomo-form-" + layout
    const layoutProps = {
      items: items,
      value: data,
    ...
    return h("el-form", { class: "lomo-form", attrs: formProps, on: on, ref: "elForm" }, [
      h(layoutName, {
        attrs: layoutProps,
