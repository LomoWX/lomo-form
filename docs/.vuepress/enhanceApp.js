
import "./styles/style.css";
import "element-ui/lib/theme-chalk/index.css";
import Element from "element-ui/lib/index";


import { lomoForm, lomoSearchPanel, lomoFormDialog, tableditorMenuColumn, lomoMapInput, lomoDynamicTags } from "../../src/index";
import renderComponent from "../../src/form/render-component";


const mockData = {
  random() {
    return "随机数" + Math.ceil(Math.random() * 100) + '' + Math.ceil(Math.random() * 100)
  },
  "/api/getRandomData": function () {
    return [{ label: this.random(), value: '1' }, { label: this.random(), value: '2' }, { label: this.random(), value: '3' }];
  },
  "/api/getRandomTreeData": function () {
    return [
      {
        label: this.random(),
        value: "1",
        children: [
          {
            label: this.random(),
            value: "1-1",
            children: [
              {
                label: this.random(),
                value: "1-1-1",
              },
            ],
          },
        ],
      },
      {
        label: this.random(),
        value: "2",
      },
    ]
  }
}

export default ({ Vue }) => {
  Vue.use(Element, { size: 'mini' });
  Vue.use(lomoForm, {
    form: {
      xs: { span: 24 }
    },
    "el-input": function () {
      return {
        placeholder: '',
        clearable: true,
      }
    },
    "xx-array-input": function () {
      return { defaultValue: [] }
    },
    "el-date-picker": function (item) {
      if (item.type == undefined || item.type == 'date' || item.type == "daterange") {
        return { valueFormat: "yyyy-MM-dd" }
      }
      if (item.type == "datetime" || item.type == "datetimerange") {
        return { valueFormat: "yyyy-MM-dd HH:mm:ss" }
      }
      if (item.type == "month" || item.type == "monthrange") {
        return { valueFormat: "yyyy-MM" }
      }
      if (item.type == "year") {
        return { valueFormat: "yyyy" }
      }
    },
  });

  Vue.component(renderComponent.name, renderComponent);
  Vue.component(lomoSearchPanel.name, lomoSearchPanel);
  Vue.component(lomoFormDialog.name, lomoFormDialog);
  Vue.component(lomoMapInput.name, lomoMapInput);
  Vue.component(tableditorMenuColumn.name, tableditorMenuColumn)
  Vue.component(lomoDynamicTags.name, lomoDynamicTags)

  Vue.prototype.$AMapKey = "56f6a360f541889552ce0aea6469a3e3";

  //模拟一个 http 请求
  Vue.prototype.$http = {
    get(url) {
      return new Promise((resolve) => {
        setTimeout(() => {
          let data = mockData[url]();
          console.log('模拟请求数据', data);
          resolve(data);
        }, 1500);
      });
    },
  }
};
